...
void bit_rev_copy_hann(const float *a, complex_t *A, int n){
    for(int i = 0; i < n; ++i){
        A[bit_rev(i)] = a[i] * (0.5f - 0.5f * std::cos(2.0f * (float)M_PI * i / n));
    }
}
...
void bit_rev_copy_vorbis(const float *a, complex_t *A, int n){
    for(int i = 0; i < n; ++i){
        float v = std::sin((float)M_PI * i / n);
        v *= v;
        A[bit_rev(i)] = a[i] * std::sin((float)M_PI * v / 2);
    }
}
...
void fft(const float *a, complex_t *A, int lg_n){
    ...
    for(int s = 1; s <= lg_n; ++s){
        int m = 1 << s;
        complex_t omega_m(std::cos(2.0f * (float)M_PI / m), std::sin(2.0f * (float)M_PI / m)), omega = 1.0;
        for(int j = 0; j < m / 2; ++j){
            for(int k = j; k < n; k += m){
...
extern float max_music_volume;
float max_music_volume = 0.7f;
...
struct fft_max_power_initer_type{
    fft_max_power_initer_type(){
        for(int i = 0; i < spectrum_length * 2; ++i){
            fft_input_signal[i] = std::sin((spectrum_length / 2) * 2.0f * (float)M_PI * static_cast<float>(i) / (spectrum_length * 2));
        }
...
void play_sound(){
    ...
    callback = [](
        ...
        // 音量の最終調整
        float ratio = (std::exp(music_volume * max_music_volume) - 1.0f) / (std::exp(1.0f) - 1.0f);
        for(int ch = 0; ch < 2; ++ch){
            for(int i = 0; i < buffer_length; ++i){
                out[i * channels + ch] *= ratio;
            }
        }
    ...
    callback = [](
        ...
        for(int ch = 0; ch < data->channels(); ++ch){
            for(std::size_t i = 0; i < frameCount; ++i){
                out[i * data->channels() + ch] *= music_volume * max_music_volume;
            }
        }
